...
export default function AdminReportsPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(item => 
      ...
      (item.nav_name || '').toLowerCase().includes(lowerQuery) ||
      (item.nav_link || '').toLowerCase().includes(lowerQuery) ||
  ...
  // Reset to page 1 when query or perPage changes
  useEffect(() => {
    setCurrentPage(1);
  }, [query, perPage]);
      ...
      <div className="flex items-start justify-between mb-6 pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            ...
            {query && <span className="ml-2">| Filtered: <span className="font-semibold text-slate-800">{filteredItems.length}</span></span>}
          </p>
        </div>
        <button
          onClick={() => {
