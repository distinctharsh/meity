  ...
  try {
    if (req.method === 'GET') {
      const [rows] = await pool.query(
        `SELECT id, platform, content, image_url, post_url, display_order, is_active, created_at, updated_at
    ...
    if (req.method === 'POST') {
      ...
      const [result] = await pool.query(
        `INSERT INTO social_media_posts (platform, content, image_url, post_url, display_order, is_active)
         VALUES (?, ?, ?, ?, ?, ?)` ,
        [platform, content, image_url, post_url, Number(display_order) || 0, !!is_active]
      );
